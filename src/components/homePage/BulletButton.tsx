...
export default function BulletButton() {
  ...
  const messages = [
    ...
    { id: 3, label: 'Our Locations', href: '/locations', icon: <OurLocations /> },
    { id: 4, label: 'Stay Connected', href: '/stay-connected', icon: <SocialsSvg /> },
  ];
  ...
  return (
    <div className="relative z-50 font-poppins">
      ...
      <button
        ...
        className="text-center w-12 h-12 text-black bg-none px-2 py-1 rounded-full flex items-center justify-center"
        onClick={toggleMenu}
      >
      ...
      {/* Sliding menu - Always in DOM, controlled by class */}
      <div
        className={`absolute  -top-16 -right-6 mt-14 w-[400px] h-[955px] rounded-3xl bg-white  flex flex-col z-50 transition-transform duration-300 ${
          ...
          {messages.map((message) => (
            <li key={message.id} className="p-4 text-center flex flex-col gap-5 items-center">
              <Link href={message.href} className="flex flex-col gap-4">
                {message.icon}
                <span className="text-center text-gray-900 font-bold text-xl">{message.label}</span>
              </Link>
              {message.id < messages.length && <hr className="border-t border-cyan-900 w-80 ml-8 z-10" />}
            </li>
          ))}
        </ul>
      </div>
